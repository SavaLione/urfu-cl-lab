...
bool check_shader_compile_status(GLuint obj)
{
    ...
    if(status == GL_FALSE)
    {
        ...
        spdlog::error("OpenGL copmile shader error: {}", &log[0]);
        return false;
    }
    return true;
}
...
bool check_program_link_status(GLuint obj)
{
    ...
    if(status == GL_FALSE)
    {
        ...
        spdlog::error("OpenGL program error: {}", &log[0]);
        return false;
    }
    return true;
}
...
int main(int argc, char *argv[])
{
    ...
    std::vector<GLubyte> image(4 * window_width * window_height);
    for(int j = 0; j < window_height; ++j)
    {
        for(int i = 0; i < window_width; ++i)
        {
            size_t index         = j * window_width + i;
    ...
    // set texture content
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, window_width, window_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, &image[0]);
